...
public class ClassicRpgGui : MonoBehaviour {
	...
	public List<string> _windowTargetTextArray = new List<string>();
	...
	private void onDialogueTextPhaseHandler(DialoguerTextData data){
		...
		_windowTargetTextArray = new List<string>(data.text.Split (' '));
		_nameText = data.name;
	...
	void OnGUI(){
		...
		float rectY = (!_usingPositionRect) ?  50 : _positionRect.y;
		float rectWidth = (!_usingPositionRect) ? 512 : _positionRect.width;
		float rectHeight = (!_usingPositionRect) ? 60 : _positionRect.height;
		...
		if(_nameText != string.Empty){
			Rect nameBoxRect = new Rect(dialogueBoxRect.x, dialogueBoxRect.y - 25, 150 * _windowTweenValue, 50 * _windowTweenValue);
...
//		drawDialogueBox(rect, new Color(45/255.0F,111f/255.0F, 255/255.0F));
		drawDialogueBox (rect, Color.black);
	}
...
//		GUI.Label(new Rect(rect.x + 1, rect.y + 2, rect.width, rect.height), text);
	...
	private void calculateText(){
		if(_windowTargetTextArray.Count == 0 || _windowTargetText == string.Empty || _windowCurrentText == _windowTargetText) return;
...
//		}
		if (_windowCurrentText != _windowTargetText) {
			...
			if (_windowTargetTextArray.Count > 0)
				_windowCurrentText += " " ;
		}
